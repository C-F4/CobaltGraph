    ...
    def add_connection(self, conn_data: Dict):
        ...
        try:
            with self.lock:
                ...
                self.conn.commit()
                logger.debug(
                    f"Connection added: {conn_data.get('dst_ip')}:{conn_data.get('dst_port')}"
                )
    ...
    def get_recent_connections(self, limit: int = 50) -> List[Dict]:
        ...
        try:
            with self.lock:
                ...
                logger.debug("Retrieved {len(results)} recent connections (limit=%s)", limit)
                return results
    ...
    def get_connection_count(self) -> int:
        ...
        try:
            with self.lock:
                ...
                logger.debug("Connection count: %s", count)
                return count
